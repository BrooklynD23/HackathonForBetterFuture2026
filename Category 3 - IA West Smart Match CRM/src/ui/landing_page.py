...
def _render_hero() -> None:
    ...
    # CTA buttons via Streamlit columns
...
def _render_product_preview() -> None:
    ...
    with col_main:
        ...
        # Factor weight previews
        factor_preview = st.columns(3)
        for i, (label, pct) in enumerate(
            [("Domain Relevance", "35%"), ("Scraped Role Fit", "25%"), ("Location", "20%")]
        ):
            with factor_preview[i]:
                st.markdown(
    ...
    for i, (title, desc) in enumerate(features):
        with cols[i]:
            st.markdown(
                ...
                f'margin-bottom: 1rem; color: #005394; font-size: 1.5rem;">'
...
def _render_factor_donut() -> None:
    """6-factor weight distribution donut chart using Plotly."""
    st.markdown(
        '<h2 style="font-family: \'Inter Tight\', sans-serif; font-weight: 700; '
        'font-size: 1.8rem; margin-bottom: 0.5rem; color: #191c1e;">'
        "The Bridge: 6-Factor MATCH_SCORE</h2>"
    ...
    with col_chart:
        ...
        opacity_steps = [1.0, 0.85, 0.7, 0.55, 0.35, 0.2]
        base_r, base_g, base_b = 0, 83, 148  # #005394
        colors = [
            f"rgba({base_r},{base_g},{base_b},{op})" for op in opacity_steps
        ]
        ...
        st.plotly_chart(fig, use_container_width=True, key="landing_donut")
...
def _render_automation_pipeline() -> None:
    ...
    with col_text:
        ...
        for i, uni in enumerate(["UCLA", "USC", "SDSU", "+42"]):
            with badge_cols[i]:
                st.markdown(
